...
//#else
//// If on other than WASP, use this name.
//void
//Log_NTDS_Mesg (
//   int        device,
//   int        category,
...
//#ifdef SOLARIS_WASP
  ...
  va_end(va);
  Log_NTDS_Mesg(category, S1);
//#else

  /* Get the UNIX Time.                                                       */
//#ifdef REMOTE_APPLICATION
//  Get_NTDS_UNIX_Time(&time.tv_sec, &time.tv_usec);
//#endif
//
//  /* Don't bother with INFOs if they aren't enabled */
//  if ((category > 0) && (Get_Current_Monitor_Log_Level() != INFO))
//    return;
//
//  /* Find the colon in the string that was just received.  It should have
//   * a form like the following:
//   *
//   *     "NIO:Get_IB-%d Some text here 0x%x"
//   *
//   */
//  colon_ptr = strchr (string_ptr, ':');
//
//  /* You have to use a colon or everything gets plugged up */
//  if (colon_ptr == NULL)
//    length = 0;
//  else
//  {
//    /* Compute the length of the string before the colon */
//    length = (unsigned int) (colon_ptr - string_ptr);
//    /* Extract the string to do a compare later. */
//    strncpy (mod, string_ptr, length);
//  }
//
//  mod[length] = '\0';
//
...
//  i_mod = 0;
//
//  /* Search the master list until you find a match or hit the end.            */
//  /* We can use the computed index to pass the string to the log manager.     */
//  while (!done && (i_mod < MAX_MOD_STRING_LIST))
//  {
//    if (strcmp (mod, module_string[i_mod]) == 0)
//    {
//      done = TRUE;
//    }
//    else
...
//    }
//  }
//  mod_ptr = (char*)module_string[i_mod];
//
//  if (category > 0)
//    severity = INFO;
//  else
...
//    strcpy(S1, "UNK");
//  else
//    vsprintf(S1, (colon_ptr + 1), va);
//  if (strcmp (string_ptr, "REMOTE") == 0)
//    vLog_Appl_Msg ((LOG_LEVELS)severity, mod_ptr, "", S1, va);
//  else
//  {
//    vLog_Appl_Msg ((LOG_LEVELS)severity, mod_ptr,
//      category_string[category], S1, va);
//  }
//  va_end(va);
//
//#ifdef REMOTE_APPLICATION
//  gmtime_r((unsigned long*)&time.tv_sec, &utime_info);
//  sprintf(S2,"%3d-%02d:%02d:%02d.%03d %s",
//    utime_info.tm_yday + 1, utime_info.tm_hour, utime_info.tm_min,
//    utime_info.tm_sec,  (int)((time.tv_usec / 1000) % 1000),  S1);
//
//  /* Send the String to the Enet Server to send to the Application.          */
//  if (device != 0)
//    Queue_Remote_Enet_Log_Mesg (device, S2);
//  else
//    Brdcast_Remote_Enet_Log_Mesg(S2);
//#endif
//
//#endif //SOLARIS_WASP
  return;
